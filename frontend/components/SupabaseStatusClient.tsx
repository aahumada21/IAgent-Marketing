...
export default function SupabaseStatusClient({ orgId }: { orgId: string | null }) {
  ...
  useEffect(() => {
    ...
    const { data: subscription } = supabase.auth.onAuthStateChange((evt) => {
      pushLog("info", `Auth event: ${evt}`);
    });

    return () => {
      subscription.subscription.unsubscribe();
    };
    // `supabase` es un singleton importado; no hace falta en deps.
    ...
    <div className="flex items-center gap-3">
      ...
      >
        <span
          ...
          style={{ background: dbOk === "ok" ? "#10b981" : dbOk === "fail" ? "#ef4444" : "#9ca3af" }}
        />
      ...
      <details className="relative">
        <summary className="cursor-pointer list-none rounded border px-2 py-1 text-xs hover:bg-gray-50">Log</summary>
