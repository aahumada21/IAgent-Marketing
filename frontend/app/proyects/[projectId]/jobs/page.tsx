  ...
  export default function ProjectJobsPage() {
    ...
    async function load() {
      ...
      setError(null);
      try {
        // 1) total (count) - usa head:true para no traer filas
        ...
        <header className="flex flex-wrap items-center gap-3">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Proyecto: <code className="text-gray-800">{projectId}</code>
            </p>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <a
              href={`/ads?project_id=${encodeURIComponent(
                String(projectId)
              )}`}
  ...
  function MediaThumb({ url, jobType }: { url: string | null; jobType: Job["job_type"] }) {
    ...
    // Para Sección A guardamos una imagen base; si algún job futuro usa video, podrías cambiar aquí.
    if (jobType === "video") {
      return (
        <video
          src={url}
